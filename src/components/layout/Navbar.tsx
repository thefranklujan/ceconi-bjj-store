...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 bg-brand-black/95 backdrop-blur border-b border-brand-gray">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            {/* Cart + Member Login (desktop) + Mobile hamburger */}
            <div className="flex items-center gap-3">
              ...
              <Link
                href="/members/login"
                className="hidden md:inline-block bg-brand-teal text-brand-black text-xs font-bold px-4 py-2 rounded uppercase tracking-wider hover:bg-brand-teal/90 transition"
              >
                Member Login
              </Link>
      ...
      {/* Drawer */}
      <div
        className={`fixed top-0 right-0 z-[70] h-full w-72 bg-brand-dark border-l border-brand-gray shadow-2xl transform transition-transform duration-300 ease-in-out md:hidden ${
      ...
      >
        ...
        <nav className="p-5 space-y-1">
          <Link
            href="/"
            className="block px-3 py-3 text-white font-medium uppercase tracking-wider text-sm hover:text-brand-teal hover:bg-brand-gray/50 rounded-lg transition"
            onClick={() => setMenuOpen(false)}
          >
            Home
          </Link>
          ...
          <Link
            href="/cart"
            className="flex items-center gap-2 px-3 py-3 text-white font-medium uppercase tracking-wider text-sm hover:text-brand-teal hover:bg-brand-gray/50 rounded-lg transition"
            onClick={() => setMenuOpen(false)}
          >
            Cart
            {itemCount > 0 && (
              <span className="bg-brand-teal text-brand-black text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">
                {itemCount}
              </span>
            )}
          </Link>
          ...
          <div className="pt-4 pb-2">
            <span className="px-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Categories</span>
          </div>
          {categories.map((cat) => (
            <Link
              key={cat}
              href={`/categories/${cat.toLowerCase().replace(/\s+/g, "-")}`}
              className="block px-3 py-2 text-sm text-gray-300 hover:text-brand-teal hover:bg-brand-gray/50 rounded-lg transition"
              onClick={() => setMenuOpen(false)}
            >
              {cat}
            </Link>
        ...
        {/* Member Login at bottom */}
        <div className="absolute bottom-0 left-0 right-0 p-5 border-t border-brand-gray">
          <Link
            href="/members/login"
            className="block w-full text-center bg-brand-teal text-brand-black text-sm font-bold px-4 py-3 rounded-lg uppercase tracking-wider hover:bg-brand-teal/90 transition"
            onClick={() => setMenuOpen(false)}
          >
            Member Login
          </Link>
